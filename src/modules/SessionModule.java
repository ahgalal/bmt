...
public class SessionModule extends Module {
	...
	private SessionModuleConfigs session_configs;

	public SessionModule(String name,ModuleConfigs config) {
		super(name, config);
	...
	{
		long totalTime = (session_end_time - session_start_time)/(1000);// total time in seconds
		return totalTime;
	}

	private float getSessionTimeTillNow()
	{
		long time = (System.currentTimeMillis()- session_start_time)/(1000);// time in seconds
		return time;
	}
