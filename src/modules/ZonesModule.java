...
public class ZonesModule extends Module {
	...
	private ZonesModuleConfigs zones_configs;

	public ZonesModule(String name,ZonesModuleConfigs configs)
	{
		super(name,configs);
	...
	private void zoneHysteresis()
	{
		if(current_zone_num!= updated_zone_number & updated_zone_number!=-1)
		{
			...
			try
			{
				zone_up_left = zone_controller.getZone(current_position.x-zones_configs.hyst_value/2,	current_position.y+zones_configs.hyst_value/2);
				zone_up_right = zone_controller.getZone(current_position.x+zones_configs.hyst_value/2,	current_position.y+zones_configs.hyst_value/2);
				zone_down_left = zone_controller.getZone(current_position.x-zones_configs.hyst_value/2,	current_position.y-zones_configs.hyst_value/2);
				zone_down_right = zone_controller.getZone(current_position.x+zones_configs.hyst_value/2,current_position.y-zones_configs.hyst_value/2);
			}
			catch(Exception e)
			{
				PManager.log.print("Error fel index .. zoneHysteresis!",this,StatusSeverity.ERROR);
			}
