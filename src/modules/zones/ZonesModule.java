 ...
 * @author Creative
 */
public class ZonesModule extends Module<ZonesModuleGUI,ZonesModuleConfigs,ZonesModuleData>
{
	...
	public void updateZoneMap()
	{
		...
		{
			tmp_zone_number = data.zones.getZoneByNumber(i)
					.getZoneNumber();
				...
				& updated_zone_number != -1)
		{
			...
			try
			{
				zone_up_left = getZone(
						current_position.x - configs.hyst_value / 2,
						current_position.y + configs.hyst_value / 2);
			...
			if (zone_up_left != data.current_zone_num
					...
					& zone_down_right != data.current_zone_num) // we
			// are
			// in
			// a
			// new
	...
	private void updateZoneCounters()
	{
		...
		if (data.zones.getZoneByNumber(data.current_zone_num) != null)
			if (data.zones.getZoneByNumber(
					data.current_zone_num).getZoneType() == ZoneType.CENTRAL_ZONE)
	...
	private void updateCentralZoneTime()
	{
		if (data.zones.getNumberOfZones() != -1)
		{
			if (data.current_zone_num != -1
				...
				else if (data.zones.getZoneByNumber(
						data.current_zone_num).getZoneType() == ZoneType.CENTRAL_ZONE
						&& data.central_flag)
