...
/**
 * @author ShaQ Handles all the experiments info
 */
public class Experiment implements Exp2GUI
{
	 ...
	 * @param user
	 *            the user name who makes the experiment
	 * @param date2
	 ...
	 * stream // connected to "myfile.txt" p.println("Experiment Info :");
	 * p.println("Experiment Name" + '\t' + "User Name" + '\t' +
	 * "Number Of Groups" + '\t' + "Date" + '\t' + "Notes"); p.println(name +
	 * '\t' + user + '\t' + no_groups + '\t' + date + '\t' + notes);
	 * p.println("Experiment Name : " + name); p.println("User Name : " + user);
	 * p.println("Number Of Groups : " + no_groups); p.println("Date : " +
	 * date); p.println("Notes : " + notes); p.close(); } catch (Exception e) {
	 * e.printStackTrace(); } }
	 */
	 ...
	 * 
	 * @param g
	 *            the reference to a group to add to the experiment
	 */
	...
	/*
	 * public void addNewGroup(String name, int no_rats, String rats_numbering,
	 * String notes){ model.Group g = new
	 * model.Group(name,no_rats,rats_numbering,notes); groups.add(g); }
	 */
	...
	public void setGroups(final ArrayList<model.business.Group> groups)
	{
		this.groups = groups;
	}
