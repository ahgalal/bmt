...
public class JMFModule implements VidInputter {
	...
	public JMFModule(String format,int w,int h) {
		this.format = format;
		width=w;
		height=h;
	}
	...
	private MediaLocator obtainMediaLocator(VideoFormat format_to_use)
	{
	...
	//@SuppressWarnings("unchecked")
	private boolean initializeJMF(FrameIntArray fia)
	{
		try {
			//MainGUI.getDefault().createAWTFrame();
			MediaLocator ml = null;

			RGBFormat rgb640x480 = new RGBFormat(new Dimension(width, height), width*height*3, RGBFormat.byteArray, 30.0f, 24, -1, -1, -1,-1,-1,-1,-1);

			YUVFormat yuv640x480_device = new YUVFormat(new Dimension(width, height),width*height*2, RGBFormat.byteArray, 30.0f, 32, 1280 , 1280 , 0,1,3);
			//YUVFormat yuv640x480 = new YUVFormat(new Dimension(352, 288),-1, RGBFormat.byteArray, -1, 2, -1 , -1 , -1,-1,-1);

			//RGBFormat rgb320x240 = new RGBFormat(new Dimension(320, 240), RGBFormat.NOT_SPECIFIED, RGBFormat.byteArray, -1, 24, 3, 2, 1,-1,-1,-1,-1);
			...
			if (format == "YUV") format_to_use=yuv640x480_device;
			else if (format == "RGB") format_to_use = rgb640x480;
			...
			try {
				...
				return false;
				//MsgBox.show(MainGUI.getDefault().getsShell(), "Error with CAM\n" + e1.getMessage(), "Error", SWT.ERROR);
			}

			/*
			 * formatcontrol is an array of FormatControl ... this array contains
			 * all FormatControls of the specified DataSource"Casted as CaptureDeviceInfo"
			 * ex: if we have a single channel source (video no audio) .. so,
			 * we will have only one element inside the formatcontrol array , this
			 * element represents the FormatControl of this Video Channel.
			 * we can then change the Format of this Video Channel by using:
			 * formatcontrol[0].set.setFormat(video_format);
			 * also, we can get a list of supported Formats by this Channel using:
			 * formatcontrol[0].getSupportedFormats()
			 */
			System.err.print("0");
			FormatControl[] formatcontrol = ((CaptureDevice)ds).getFormatControls();
			Format[] supported_formats =formatcontrol[0].getSupportedFormats();
			...
			TrackControl[] tracks = proc_1.getTrackControls();
			//proc_1.setContentDescriptor(null);
			System.err.print("2");



			ana_eff = new AnalysisEffect(video_format.getSize().width,video_format.getSize().height,fia);
			ana_eff.setInputFormat(tracks[0].getFormat());
			ana_eff.setOutputFormat(new RGBFormat (new Dimension(width, height), width*height*3, RGBFormat.byteArray, 30.0f, 24, 3, 2, 1,3,width*3,0,0));

			System.err.print("3");

			Codec[] codecs = {ana_eff};
			try {
			...
			proc_1.realize();
			while(proc_1.getState()!=Processor.Realized);
	...
	public boolean StartStream() {
		...
		/*						DataSource ds2 = ((SourceCloneable)ds).createClone();


		try {
			Player pl2 = Manager.createRealizedPlayer(ds2);
			pl2.realize();
		...
		try {
			pl.start();
			while(pl.getState()!=Player.Started);
			System.err.print("8");
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		//frameGrabber = (FrameGrabbingControl)proc_1.getControl("javax.media.control.FrameGrabbingControl");
	}
	...
	public void StopModule() {
		...
		if(proc_1!=null)
		{
			proc_1.stop();
			proc_1.close();
			while((proc_1.getState()==Processor.Started));
			proc_1=null;
		}
		if(pl!=null)
		{
