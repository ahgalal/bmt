...
public class AverageFilter extends VideoFilter
{
	...
	public AverageFilter(final String name, final Link link_in, final Link link_out)
	{
	...
	public void process()
	{
		if (configs.enabled)
		{
			...
			{
				for (int y = 20; y < configs.common_configs.height - maskHeight - 20; y += yStepping)
				{
					maskAverage = 0;
					for (int xMask = x; xMask < x + maskWidth; xMask++)
					{
						...
						{
							maskAverage = ImageManipulator.addRGBInt(dataIn[xMask
									+ configs.common_configs.width
									* yMask], maskAverage);
						}
					}
					int val = ImageManipulator.divideRGBByNumber(
							maskAverage,
							maskArea);
					dataOut[x
							+ maskWidth
