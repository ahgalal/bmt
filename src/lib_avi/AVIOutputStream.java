/**
 * @(#)AVIOutputStream.java 1.3.1 2010-07-19 Copyright (c) 2008-2010 Werner
 *                          Randelshofer Hausmatt 10, CH-6405 Immensee,
 *                          Switzerland All rights reserved. The copyright of
 *                          this software is owned by Werner Randelshofer. You
 *                          may not use, copy or modify this software, except in
 *                          accordance with the license agreement you entered
 *                          into with Werner Randelshofer. For details see
 ...
 * set by setting an approprate pair of values using methods
 ...
 * @version 1.3.1 2010-07-19 Fixes seeking and calculation of offsets. <br>
...
public class AVIOutputStream
{
	...
	private static class Sample
	{
		 ...
		 * @param length
		 */
		public Sample(final int duration, final long offset, final long length)
		{
	...
	private abstract class Chunk
	{
		 ...
		 *            characters.
		 */
		public Chunk(final String chunkType) throws IOException
		{
	...
	private class CompositeChunk extends Chunk
	{
		...
		public void add(final Chunk child) throws IOException
		{
			if (children.size() > 0)
			{
		...
		public void finish() throws IOException
		{
			if (!finished)
			{
				if (size() > 0xffffffffL)
				{
					throw new IOException("CompositeChunk \""
							+ chunkType
							+ "\" is too large: "
							+ size());
				}

				final long pointer = getRelativeStreamPosition();
				seekRelative(offset);

				final DataChunkOutputStream headerData = new DataChunkOutputStream(
				...
				for (final Chunk child : children)
				{
					child.finish();
				}
	...
	private class DataChunk extends Chunk
	{
		 ...
		 */
		public DataChunk(final String name) throws IOException
		{
			super(name);
			out.writeLong(0); // make room for the chunk header
		...
		public void finish() throws IOException
		{
			if (!finished)
			{
				final long sizeBefore = size();

				if (size() > 0xffffffffL)
				{
					throw new IOException("DataChunk \""
							+ chunkType
							+ "\" is too large: "
							+ size());
				}
	...
	private class FixedSizeDataChunk extends Chunk
	{
				...
				throws IOException
		{
			...
			final byte[] buf = new byte[(int) Math.min(512, fixedSize)];
			long written = 0;
	...
	/**
	 * Creates a new AVI file with the specified video format and framerate.
	 * 
	 * @param file
	 *            the output file
	 * @param format
	 ...
	 */
	public AVIOutputStream(final File file, final VideoFormat format) throws IOException
	{
		if (format == null)
		{
	 ...
	 * @param newValue
	 */
	public void setTimeScale(final int newValue)
	{
		if (newValue <= 0)
		{
	 ...
	 * @param newValue
	 */
	public void setFrameRate(final int newValue)
	{
		if (newValue <= 0)
		{
	 ...
	 * @param newValue
	 */
	public void setVideoCompressionQuality(final float newValue)
	{
	 ...
	 *            Must be greater than 0.
	 */
	public void setVideoDimension(final int width, final int height)
	{
		if (width < 1 || height < 1)
		{
	 ...
	 */
	public void writeFrame(final BufferedImage image) throws IOException
	{
		ensureOpen();
		ensureStarted();
		...
		final DataChunk videoFrameChunk = new DataChunk(
				(videoFormat == VideoFormat.RAW) ? "00db" : "00dc");
		moviChunk.add(videoFrameChunk);
		final long offset = getRelativeStreamPosition();
		switch (videoFormat)
		{
		case RAW:
		{
			final WritableRaster raster = image.getRaster();
			final int[] raw = new int[imgWidth * 3]; // holds a scanline of raw
			// image
			// data with 3 channels of 32
			// bit data
			final byte[] bytes = new byte[imgWidth * 3]; // holds a scanline of
			// raw
			// image data with 3
		...
		case JPG:
		{
			...
			final MemoryCacheImageOutputStream imgOut = new MemoryCacheImageOutputStream(
		...
		default:
		{
			final ImageWriter iw = ImageIO.getImageWritersByMIMEType("image/png").next();
			final ImageWriteParam iwParam = iw.getDefaultWriteParam();
			final MemoryCacheImageOutputStream imgOut = new MemoryCacheImageOutputStream(
					videoFrameChunk.getOutputStream());
			iw.setOutput(imgOut);
		...
		final long length = getRelativeStreamPosition() - offset;
	 ...
	 *             if writing the image failed.
	 */
	public void writeFrame(final File file) throws IOException
	{
	 ...
	 *             if writing the image failed.
	 */
	public void writeFrame(final InputStream in) throws IOException
	{
		ensureOpen();
		ensureStarted();

		final DataChunk videoFrameChunk = new DataChunk("00dc");
		moviChunk.add(videoFrameChunk);
		final OutputStream mdatOut = videoFrameChunk.getOutputStream();
		final long offset = getRelativeStreamPosition();
	 ...
	 */
	private void seekRelative(final long newPosition) throws IOException
	{
	...
	private void writeProlog() throws IOException
	{
		...
		final CompositeChunk hdrlChunk = new CompositeChunk("LIST", "hdrl");
		...
		final CompositeChunk strlChunk = new CompositeChunk("LIST", "strl");
	...
	private void writeEpilog() throws IOException
	{
		// Compute values
		int duration = 0;
		for (final Sample s : videoFrames)
		{
			duration += s.duration;
		}
		long bufferSize = 0;
		 ...
		 * aIndex[]; } AVIOLDINDEX;
		 */
		final DataChunk idx1Chunk = new DataChunk("idx1");
		aviChunk.add(idx1Chunk);
		d = idx1Chunk.getOutputStream();

		for (final Sample f : videoFrames)
		{
