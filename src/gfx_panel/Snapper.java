...
public class Snapper
{
	...
	private SnapResults snp_results;
	private ArrayList<Shape> arr_shp;
	public Snapper(ArrayList<Shape> arr_shp) {
	...
	public SnapResults snap(SnapInfo snapInfo)
	{
		Shape shp_snap_to= snapInfo.shp_snap_to,
		shp_snap=snapInfo.shp_snap;
	...
	public SnapResults prepareSnapPosition(Shape shp,int cur_x,int cur_y,
			int cursor_pos_in_shp_x,int cursor_pos_in_shp_y)
	{
		if(enable_snap)
		{
			snp_results.snapped_x=snp_results.snapped_y=false;
			int edge_x=cur_x-cursor_pos_in_shp_x,
			edge_y=cur_y-cursor_pos_in_shp_y,
			dist_to_tmpshp_x,dist_to_tmpshp_y,
			shp_x=shp.getX(),shp_y=shp.getY(),shp_w=shp.getWidth(),shp_h=shp.getHeight(),
			tmp_x,tmp_y,tmp_w,tmp_h;
			for(Shape tmp_sh:arr_shp)
			{
				if(tmp_sh!=shp)
				{
					tmp_x= tmp_sh.getX();tmp_y=tmp_sh.getY();
					tmp_w=tmp_sh.getWidth();tmp_h=tmp_sh.getHeight();

					dist_to_tmpshp_x=edge_x - tmp_x - tmp_w;
					dist_to_tmpshp_y=edge_y - tmp_y - tmp_h;

					ArrayList<SnapInfo> snap_array_x=new ArrayList<SnapInfo>();
					ArrayList<SnapInfo> snap_array_y=new ArrayList<SnapInfo>();
					...
					)
					{
						...
						if(edge_x > tmp_x +tmp_w)
						{
							...
							if( dist_to_tmpshp_x< 10 & dist_to_tmpshp_x > 0)
								snap_array_x.add(new SnapInfo(shp,tmp_sh,Direction.RIGHT,dist_to_tmpshp_x));
						}
						else if(edge_x +shp_w < tmp_x)
						{
							...
							if( dist_to_tmpshp_x< 10 & dist_to_tmpshp_x > 0)
								snap_array_x.add(new SnapInfo(shp,tmp_sh,Direction.LEFT,dist_to_tmpshp_x));
						}
						else
					...
					)
					{
						...
						if(edge_y > tmp_y + tmp_h)
						{
							...
							if( dist_to_tmpshp_y< 10 & dist_to_tmpshp_y > 0)
								snap_array_y.add(new SnapInfo(shp,tmp_sh, Direction.DOWN, dist_to_tmpshp_y));
						}
						else if(edge_y + shp_h < tmp_y)
						{
							...
							if( dist_to_tmpshp_y< 10 & dist_to_tmpshp_y > 0)
								snap_array_y.add(new SnapInfo(shp,tmp_sh, Direction.UP, dist_to_tmpshp_y));
						}
						else
							/* cursor is in the middle of tmp, we must set
					...
					{
						//SNAP_Y:
						smaller_dist=1000;smaller_dist_idx = 0;
	...
	public SnapResults prepareSnapSize(Shape shp,int x,int y)
	{
		if(enable_snap)
		{
			snp_results.snapped_x=snp_results.snapped_y=false;
	...
	private class SnapInfo
	{
		...
		public SnapInfo(Shape shp_to_snap,Shape shp_to_snap_to,Direction direction,int distance)
		{
