...
/**
 * Parent of all Shapes. Rectangles,Ovals and others inherite from this class.
 * a Shape can be drawn on GfxPanel.
 * @author Creative
 */
public abstract class Shape {
	...
	protected int selectionbox_length;			//side length of the selection box drawn at corners
	protected ArrayList<Shape> arr_attachee;	//array of attached shapes to this shape, they move as this shape moves
	
	/**
	 * Draw the Shape on the graphics object.
	 * @param g Graphics object to draw the shape on 
	 */
	 ...
	 * @param width 
	 * @param height
	 * @param c shape's color
	 */
	public Shape(int x, int y, int width, int height,RGB c) {
		this.x = x;
		this.y = y;
		this.width = width;
		this.height = height;
	...
	public void setShape_number(int shape_number) {
	 ...
	 * @param length side length of the selection box
	 */
	public void select(Graphics gfx,int length)
	{
	...
	public void deselect(Graphics gfx)
	{
	 ...
	 * @param strcol String containing the color's name
	 ...
	 * attachees are moved.
	 * @param attachee Shape to attach
	 */
	public void attachToMe(Shape attachee)
	{
	 ...
	 * @param attachee Shape to deattach
	 */
	public void deattachFromMe(Shape attachee)
	{
		arr_attachee.remove(attachee);
	}

	/**
	 * Converts RGB color objects to a string value of:
	 * R
	 * G
	 * B
	 * @param color
	...
	public static String color2String(RGB color) {
		return Integer.toString(color.red) + System.getProperty("line.separator")+ Integer.toString(color.green) + System.getProperty("line.separator")+Integer.toString(color.blue) ;
	}


	/**
	 * Move this shape to certain X position, move the attachees also.
	 * @param x new X value to move to
	 */
	public void setX(int x) {
		for(Shape att:arr_attachee)
			att.moveX(x-this.x);
		this.x = x;
	}
	
	/**
	 * Moves this shape by a value delta_x.
	 * if delta_x is positive, shape moves right, else , to the left.
	 * @param delta_x distance to move the shape along the x-axis
	 */
	public void moveX(int delta_x)
	{
		setX(x+delta_x);
	}

	/**
	 * Moves this shape by a value delta_y.
	 * if delta_y is positive, shape moves down, else , up.
	 * @param delta_y distance to move the shape along the y-axis
	 */
	public void moveY(int delta_y)
	{
		setY(y+delta_y);
	}
	 ...
	 * @param y new Y value to move to
	 */
