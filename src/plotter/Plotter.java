...
public class Plotter
{
	...
	private Point shiftAxis(int x,int y)
	{
	...
	public void initialize()
	{
		...
		mainComposite.setBounds(0, 0, parent.getBounds().width, parent.getBounds().height);
		
		canvas = new Canvas(mainComposite, 0);
		gc = new GC(canvas);
		canvas.setBounds(0, 0, mainComposite.getSize().x,mainComposite.getSize().y);
		...
		btnNewButton.addSelectionListener(new SelectionAdapter() {
			...
			{
				/*				addPoint(20, 20);
								addPoint((int)(Math.random()*200), (int)(Math.random()*200));
								addPoint((int)(Math.random()*200), (int)(Math.random()*200));
								addPoint((int)(Math.random()*200), (int)(Math.random()*200));*/
				drawAxes();
			}
	...
	{
		gc.drawLine(axisOffsetX, 10, axisOffsetX, canvas.getBounds().height - 10);
		gc.drawLine(
				10,
				canvas.getBounds().height - axisOffsetY,
				canvas.getBounds().width - 10,
				canvas.getBounds().height - axisOffsetY);
	}
