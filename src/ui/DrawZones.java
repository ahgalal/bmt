...
public class DrawZones extends BaseUI {
	...
	private void createComposite() {
		...
		gfx_panel = new GfxPanel(sShell, composite, composite.getSize().x, composite.getSize().y);
	...
	private void createSShell() {
		...
		.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				MessageBox msgbox = new MessageBox(sShell);
				...
				Thread th_2points = new Thread(new Runnable() {
					@Override
					public void run() {
						final InputBox inputbox_getreal_dist = new InputBox(sShell,"Real Distance","Please enter the real distance between the 2 points (in cm):",false);
		...
		TableColumn tableColumn = new TableColumn(tbl_zones, SWT.NONE);
		tableColumn.setWidth(40);
		tableColumn.setText("Num");
		TableColumn tableColumn1 = new TableColumn(tbl_zones, SWT.NONE);
		tableColumn1.setWidth(90);
		tableColumn1.setText("Type");
		TableColumn tableColumn2 = new TableColumn(tbl_zones, SWT.NONE);
		...
		tbl_zones.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				gfx_panel.selectShape(Integer.parseInt(tbl_zones.getItem(tbl_zones.getSelectionIndex()).getText(0)));
			}
	 ...
	 * @param zone_no zone number
	 * @param zone_col zone color
	 * @param zone_type zone type
	 */
	public void addZoneToTable(final String zone_no,final String zone_col,final String zone_type)
	{
	 ...
	 * @param zone_type zone type
	 */
	public void editZoneDataInTable(final int zone_no,final String zone_col,final String zone_type)
	{
		if(Display.getCurrent()!=Display.getDefault())
			Display.getDefault().asyncExec(new Runnable() {
	 ...
	 * @param zone_type zone type
	 */
	private void GUIeditZoneDataInTable( int zone_no, String zone_col, String zone_type)
	{
		TableItem tmp_ti = getTableItemByNumber(zone_no);
		if(tmp_ti!=null)
		{
	...
	{
		for(TableItem ti:tbl_zones.getItems())
			ti.dispose();
	 ...
	 * @param zone_type zone type
	 */
	private void GUIaddZoneToTable( String zone_no, String zone_col, String zone_type)
	{
		TableItem tmp_ti = new TableItem(tbl_zones, 0);
	...
	public void selectZoneInTable(final int zone_number)
	{
		if(Display.getCurrent()!=Display.getDefault())
			Display.getDefault().asyncExec(new Runnable() {
				...
				@Override
				public void run() {
					TableItem ti=getTableItemByNumber(zone_number);
					if(ti!=null)
	...
	private TableItem getTableItemByNumber(int zone_number)
	{
		for(TableItem ti:tbl_zones.getItems())
		{
	 ...
	 * Details:
	 * when setting the scale, user needs to select 2 points on the screen,
	 * then writes the real distance between them in the real world.
	 * so, this method adds a new measure point (the first one or the second)
	 * there is a thread responsible of handling the two points.
	 * @param pos position of the new point (x,y)
	 */
