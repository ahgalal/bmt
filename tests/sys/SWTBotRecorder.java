...
public class SWTBotRecorder
{
	...
	private class SWTListener implements Listener
	{
		...
		public void handleEvent(final Event event)
		{
			...
			if (event.widget instanceof Button && event.type == SWT.MouseDown)
			{
			...
			else if (event.widget instanceof MenuItem && event.type == SWT.Selection)
			{
				...
				generatedCode += botName
						+ "menu(\""
						+ mnuParent.getParentItem().getText()
						+ "\").menu(\""
						+ mnutm.getText()
						+ "\").click();\n";
			}
			else if (event.widget instanceof Text && event.type == SWT.KeyDown)
			{
