...
public class DrawZones extends BaseUI {
	...
	private void createComposite() {
		...
		gfx_panel = new GfxPanel(sShell, composite, composite.getSize().x,
				composite.getSize().y);
						...
						final Thread th_2points = new Thread(new Runnable() {
							...
							public void run() {
								...
								Display.getDefault().syncExec(new Runnable() {
									@Override
									public void run() {
										str_real_distance = inputbox_getreal_dist
												.show();
										gfx_panel.enableDraw(true);
									}
								});
								controller.sendScaletoStatsCtrlr(measure_pnt1,
										measure_pnt2, str_real_distance);
								measure_pnt1 = null;
								measure_pnt2 = null;
	 ...
	 *            zone type
	 */
	public void editZoneDataInTable(final int zone_no, final String zone_col,
			final String zone_type) {
		if (Display.getCurrent() != Display.getDefault())
			Display.getDefault().asyncExec(new Runnable() {
	 ...
	 *            zone type
	 */
	private void guiAddZoneToTable(final String zone_no, final String zone_col,
			final String zone_type) {
	 ...
	 *            zone type
	 */
	private void guiEditZoneDataInTable(final int zone_no,
			final String zone_col, final String zone_type) {
